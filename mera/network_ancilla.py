...
class Network:
    ...
    def __init__(self, num_pixels, deph_p, num_anc, init_std, lr, config):
        ...
        self.bottleneck = num_anc == 1
        if self.bottleneck: self.kraus_ops_half_bd = self.construct_dephasing_multiqubit_kraus(1)
    ...
    def get_network_output(self, input_batch: tf.constant):
        ...
        if not self.bottleneck: layer_out = self.run_remaining_network(layer_out, left_over)
        else: layer_out = self.run_remaining_network_with_bottleneck(layer_out, left_over)
    ...
    def run_remaining_network(self, layer_out, left_over):
    ...
    def run_remaining_network_with_bottleneck(self, layer_out, left_over):
        layer_out = self.layers[1].get_1st_iso_lay_out_bottleneck(layer_out, left_over)
        if self.deph_net: layer_out = self.dephase_1st_iso_lay_out(layer_out, bottleneck=True)

        layer_out = self.layers[2].get_2nd_ent_lay_out(layer_out)
        if self.deph_net: layer_out = self.dephase_2nd_ent_lay_out(layer_out, bottleneck=True)
        ...
        layer_out = self.layers[4].get_3rd_ent_lay_out(layer_out)
        if self.deph_net: layer_out = self.dephase_3rd_ent_lay_out(layer_out, bottleneck=True)
    ...
    def dephase_1st_iso_lay_out(self, tensor, bottleneck=False):
        '''
        ...
        kraus_ops = self.kraus_ops_1_bd if not bottleneck else self.kraus_ops_half_bd
        for i in range(8):
            contract_str = 'X'+u[i]+l[i]+', Z'+l[:16]+', X'+u[8+i]+l[8+i]+' -> Z'+l[:i]+u[i]+l[i+1:8+i]+u[8+i]+l[9+i:16]
            tensor = tf.einsum(contract_str, kraus_ops, tensor, kraus_ops)
        return tensor

    def dephase_2nd_ent_lay_out(self, tensor, bottleneck=False):
        '''
    ...
    def dephase_3rd_ent_lay_out(self, tensor, bottleneck=False):
        '''
        There are left-over bonds already dephased in this input tensor;
        The left-over bonds are the left-most and the right-most ones.
        :param tensor: single tensor with canonical indices
...
class Iso_Layer(Ent_Layer):
    ...
    def get_1st_iso_lay_out_bottleneck(self, inputs, left_over_data_inputs):
        ...
        contracted = tf.einsum('zayiyfh', tf.reshape(contracted, shapes))
        ...
        for i in range(1, num_nodes-1):
            ...
            contracted = tf.einsum('Z'+l[:2*i]+'AYIYFH', tf.reshape(contracted, shapes))
        ...
        output = tf.einsum('Z'+l[:2*last]+'AYGY', tf.reshape(output, shapes))
