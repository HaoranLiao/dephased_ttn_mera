...
class Layer:
    def __init__(self, num_nodes, layer_idx, num_anc, init_mean, init_std):
        ...
        self.param_var_fixed_lay = tf.Variable(
            tf.random_normal_initializer(mean=0, stddev=10)(
                shape=[self.num_op_params, num_nodes], dtype=tf.float32,
            ), name='param_var_fixed_lay_%s' % self.layer_idx, trainable=True)

    def apply_fixed_unitary(self, unitary_matrix):
        num_off_diags = int(0.5 * (self.num_op_params - self.num_diags))
        real_off_params = self.param_var_fixed_lay[:num_off_diags]
        imag_off_params = self.param_var_fixed_lay[num_off_diags:2 * num_off_diags]
        ...
        fixed_unitary_matrix = tf.einsum('nab, nbc, ndc -> nad', eigenvectors, diag_exp_mat, tf.math.conj(eigenvectors))

        return tf.matmul(fixed_unitary_matrix, unitary_matrix)
